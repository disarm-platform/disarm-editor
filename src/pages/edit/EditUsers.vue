<template>
  <div>
      <el-button @click='save'>sacve</el-button>
    <el-table
      ref="multipleTable"
      :data="users_wp"
       @select="onchange"
...
export default Vue.extend({
  mounted() {
    ...
    const result = users.map((u) => {
      ...
      const user_permissions = perms.forEach((p) => {
        const exists_for_user = permissions.some(
          (permission) => permission.user_id === u._id && permission.value === p
        );
        u.permissions[p] = exists_for_user;
      });
      return u;
    });
  ...
  computed: {
    permission_strings() {
      ...
      this.applets.forEach((applet) => {
        list.push(types[0] + ':' + applet);
        list.push(types[1] + ':' + applet);
      });
      return list;
    },
  },
  methods: {
    ...
    save() {
      ...
      const result = this.users_wp.reduce((acc, u) => {
